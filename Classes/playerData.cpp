...
Animate *Player::getAnimation(const char *direction, const int iMax, const int F, const int times) {
    ...
    for (int i = 1; i <= iMax; i++) {
        sprintf(str, "%s %d.png", direction, i);
...
void Player::onMouseMove(Event *event) {
    ...
    if (direct[0] >= 0 && direct[1] >= 0) {
        sprintf(directString1, "right back");
        sprintf(directString2, "rightb roll");
    }
    if (direct[0] > 0 && direct[1] < 0) {
        sprintf(directString1, "right front");
        sprintf(directString2, "rightf roll");
    }
    if (direct[0] < 0 && direct[1] > 0) {
        sprintf(directString1, "left back");
        sprintf(directString2, "leftb roll");
    }
    if (direct[0] <= 0 && direct[1] <= 0) {
        sprintf(directString1, "left front");
        sprintf(directString2, "leftf roll");
    }
